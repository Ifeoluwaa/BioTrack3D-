#!/usr/bin/env python
"""
BioTrack3D++ V3 hybrid trainer: V1-anchored detector + learned BioTrack association.
...
Usage:
    python scripts/train_biotrack3d_v3.py --split 0 --epochs 10 --anchor-weights /path/to/v1_checkpoint.pth
"""
...
def train_epoch(
    ...
    for iter_idx in pbar:
        ...
        for i in range(W - 1):
            ...
            # Explicit parent-level division head loss.
            # Parents with fewer than two annotated children are UNKNOWN, not negative.
            division_targets = build_division_targets(
                pair_target, ns,
            )
            ...
            if division_logits.ndim == 3 and division_logits.shape[-1] == 1:
                division_logits = division_logits[..., 0]
        ...
        division_loss = (
            sum(division_losses) / len(division_losses)
            if division_losses else edge_loss.new_tensor(0.0)
        )

        division_consistency_loss = (
            sum(division_consistency_losses)
            / len(division_consistency_losses)
            if division_consistency_losses else edge_loss.new_tensor(0.0)
        )
...
def train(
    ...
    anchor_weights: Path | None = None,
    freeze_detector: bool = True,
    debug_window: int | None = None,
) -> UNetNodeTransformer:
    ...
    if debug_window is not None:
        train_video_data = [
            (
                ...
                [
                    w for w in windows
                    if w.t_start == debug_window
                ],
            )
            for vm, windows in train_video_data
        ]
        ...
        test_video_data = [
            ...
            for vm, windows in test_video_data
        ]

        if not train_video_data[0][1]:
            raise RuntimeError(
                f"No training window found with t_start={debug_window}"
            )

        print(
            f"Debug window selected: t_start={debug_window}",
            flush=True,
        )
    ...
    # ------------------------------------------------------------------
    # V1 anchor initialization.
    # Load the proven public V1 checkpoint into every compatible layer.
    # New BioTrack3D++ layers (e.g. LCAP-v2 / division head) may be missing
    # from the anchor checkpoint and are intentionally left trainable.
    # ------------------------------------------------------------------
    if anchor_weights is not None:
        anchor_weights = Path(anchor_weights)
        if not anchor_weights.is_file():
            raise FileNotFoundError(f"Anchor checkpoint not found: {anchor_weights}")
        anchor_state = torch.load(anchor_weights, map_location="cpu", weights_only=True)
        ...
        for k, v in anchor_state.items():
            if k.startswith("module."):
                k = k[len("module."):]

            if k.startswith("unet.module."):
                k = k.replace("unet.module.", "unet.", 1)
        ...
        print(
            f"V1 anchor loaded: {anchor_weights} | "
            f"loaded={len(cleaned_anchor)} "
        ...
        if missing:
            print(
                "  Missing (new/changed) layers:",
                missing[:12],
                flush=True,
            )
    # Protect the V1 detector while we teach the BioTrack association branch.
    if freeze_detector:
        for p in model.unet.parameters():
            p.requires_grad = False
        for p in model.detect_head.parameters():
            p.requires_grad = False
        print("Frozen V1 detector: UNet + detection head", flush=True)
    else:
        print("Detector fine-tuning ENABLED", flush=True)
    ...
    for epoch in pbar:
        ...
        edge_loss, det_loss, division_loss, division_consistency_loss = train_epoch(
            ...
            pool_kernel_um=pool_kernel_um,
            div_weight=div_weight,
            div_consistency_weight=div_consistency_weight,
            div_head_weight=div_head_weight,
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Debug training window t_start to use when --debug-video is set.",
    )
    ...
    parser.add_argument("--anchor-weights", type=str, default=None,
                        help="Proven V1 checkpoint used to initialize compatible layers.")
    parser.add_argument("--freeze-detector", dest="freeze_detector", action="store_true", default=True,
                        help="Freeze UNet + detection head (default: on).")
    parser.add_argument("--unfreeze-detector", dest="freeze_detector", action="store_false",
                        help="Allow detector fine-tuning after transfer learning.")
    parser.add_argument("--div-consistency-weight", type=float, default=0.5,
                        help="Weight for the division-consistency edge loss.")
    parser.add_argument("--div-head-weight", type=float, default=0.5,
                        help="Weight for the explicit parent-level division head loss.")
    ...
    for fold in folds:
        train(
            ...
            div_consistency_weight=args.div_consistency_weight,
            div_head_weight=args.div_head_weight,
            output_dir=Path(args.output_dir),
            sibling_aware=args.sibling_aware,
            gradient_checkpointing=args.gradient_checkpointing,
            anchor_weights=Path(args.anchor_weights) if args.anchor_weights else None,
