...
class PredictConfig:
    ...
    # V16.7 diagnostic only: save exact pre-edge neural appearance features
    # for every node in each t->t+1 pair. This never changes prediction logic.
    appearance_embeddings_dir: Path | None = None
    resume_appearance_cache: bool = False
...
def predict_video(
    ...
    # Diagnostic-only detector confidence for each retained node, in the same
    # global order as all_coords_ds. This does not affect detection or tracking.
    all_detection_prob: list[np.ndarray] = []
    ...
    # V16.7 appearance cache. Store node features by adjacent-frame pair and
    # role (0=source at t, 1=target at t+1). A middle-frame node therefore
    # intentionally has two contextual embeddings: one as a target and one as
    # a source. These are exactly the features passed into predict_edges.
    app_pair_source_time: list[np.ndarray] = []
    app_role: list[np.ndarray] = []
    app_node_global: list[np.ndarray] = []
    app_embedding: list[np.ndarray] = []
    ...
    for window_start in tqdm(
        ...
        for frame_local, time_index in enumerate(frame_indices):
            ...
            # Preserve the detector probability at each retained local maximum.
            # Same ordering as detections_ds/global node indices. Diagnostic only.
            if len(detections_ds):
                det_logits_frame = detection_logits[frame_local][0, 0]
                zz = detections_ds[:, 1].astype(np.int64)
                yy = detections_ds[:, 2].astype(np.int64)
                xx = detections_ds[:, 3].astype(np.int64)
                retained_logits = det_logits_frame[zz, yy, xx]
                retained_probs = torch.sigmoid(retained_logits).detach().cpu().numpy()
                all_detection_prob.append(retained_probs.astype(np.float32, copy=False))
            else:
                all_detection_prob.append(np.empty(0, dtype=np.float32))
        ...
        for frame_local in range(window_size - 1):
            ...
            if config.appearance_embeddings_dir is not None:
                def _feature_rows(tensor, expected_rows: int) -> np.ndarray:
                    ...
                    if arr.shape[0] != expected_rows:
                        raise RuntimeError(
                            f"Appearance feature row mismatch: got {arr.shape}, "
                            f"expected first dimension {expected_rows}"
                        )
                    return arr.reshape(expected_rows, -1)

                sf = _feature_rows(source_features, n_source)
                tf = _feature_rows(target_features, n_target)

                app_pair_source_time.append(
                    np.full(n_source, source_time, dtype=np.int32)
                )
                app_role.append(np.zeros(n_source, dtype=np.uint8))
                app_node_global.append(source_global.astype(np.int64, copy=False))
                app_embedding.append(sf)

                app_pair_source_time.append(
                    np.full(n_target, source_time, dtype=np.int32)
                )
                app_role.append(np.ones(n_target, dtype=np.uint8))
                app_node_global.append(target_global.astype(np.int64, copy=False))
                app_embedding.append(tf)

                # Cache-only mode stops here: no edge/division logits, ILP or
                # rescue are needed to collect appearance features.
                if config.embedding_cache_only:
                    continue
    ...
    node_detection_prob = (
        np.concatenate(all_detection_prob).astype(np.float32, copy=False)
        if all_detection_prob else np.empty(0, dtype=np.float32)
    )
    if len(node_detection_prob) != len(coords_ds):
        raise RuntimeError(
            f"Detector-confidence cache misaligned: {len(node_detection_prob)} "
            f"scores for {len(coords_ds)} nodes"
        )

    association_candidate_cache = {
        "node_detection_prob": node_detection_prob,
        "source_global": (
        ...
        "appearance_pair_source_time": (
            np.concatenate(app_pair_source_time).astype(np.int32, copy=False)
            if app_pair_source_time else np.empty(0, dtype=np.int32)
        ),
        "appearance_role": (
            np.concatenate(app_role).astype(np.uint8, copy=False)
            if app_role else np.empty(0, dtype=np.uint8)
        ),
        "appearance_node_global": (
            np.concatenate(app_node_global).astype(np.int64, copy=False)
            if app_node_global else np.empty(0, dtype=np.int64)
        ),
        "appearance_embedding": (
            np.concatenate(app_embedding, axis=0).astype(np.float32, copy=False)
            if app_embedding else np.empty((0, 0), dtype=np.float32)
        ),
    }
...
def predict(
    ...
    for name in tqdm(
        ...
        if config.appearance_embeddings_dir is not None:
            appearance_dir = Path(config.appearance_embeddings_dir)
            appearance_dir.mkdir(parents=True, exist_ok=True)
            appearance_cache_path = appearance_dir / f"{name}.npz"
            if config.resume_appearance_cache and appearance_cache_path.exists():
                try:
                    with np.load(appearance_cache_path) as chk:
                        required = {
                            "node_id", "node_time", "node_zyx",
                            "node_detection_prob", "pair_source_time",
                            "role", "pair_node_id", "embedding",
                        }
                        if required.issubset(set(chk.files)) and len(chk["embedding"]):
                            print(
                                f"[APPEARANCE CACHE SKIP] {name}: {appearance_cache_path}",
                                flush=True,
                            )
                            continue
                except Exception:
        ...
        if config.appearance_embeddings_dir is not None:
            app_time = association_candidate_cache["appearance_pair_source_time"]
            app_role_arr = association_candidate_cache["appearance_role"]
            app_global = association_candidate_cache["appearance_node_global"]
            app_embedding_arr = association_candidate_cache["appearance_embedding"]

            if len(app_global) != len(app_embedding_arr):
                raise RuntimeError(
                    f"Appearance cache misaligned for {name}: "
                    f"globals={len(app_global)} embeddings={len(app_embedding_arr)}"
                )

            pair_node_id = np.fromiter(
                (int(node_ids[int(i)]) for i in app_global),
                dtype=np.int64,
                count=len(app_global),
            )
            node_id_arr = np.asarray([int(x) for x in node_ids], dtype=np.int64)
            node_time = coords_original[:, 0].astype(np.int32, copy=False)
            node_zyx = coords_original[:, 1:].astype(np.float32, copy=False)
            node_detection_prob = association_candidate_cache["node_detection_prob"]

            if appearance_cache_path is None:
                appearance_dir = Path(config.appearance_embeddings_dir)
                appearance_dir.mkdir(parents=True, exist_ok=True)
                appearance_cache_path = appearance_dir / f"{name}.npz"

            np.savez_compressed(
                appearance_cache_path,
                node_id=node_id_arr,
                node_time=node_time,
                node_zyx=node_zyx,
                node_detection_prob=node_detection_prob.astype(np.float32, copy=False),
                pair_source_time=app_time,
                role=app_role_arr,
                pair_node_id=pair_node_id,
                embedding=app_embedding_arr.astype(np.float32, copy=False),
            )
            print(
                f"[APPEARANCE CACHE] {name}: nodes={len(node_id_arr)} "
                f"role_rows={len(pair_node_id)} dim="
                f"{(app_embedding_arr.shape[1] if app_embedding_arr.ndim == 2 else 0)} "
                f"path={appearance_cache_path}",
                flush=True,
            )
        ...
        if config.association_candidates_dir is not None:
            ...
            node_detection_prob = association_candidate_cache["node_detection_prob"]
            source_detection_prob = node_detection_prob[sg].astype(np.float32, copy=False)
            target_detection_prob = node_detection_prob[tg].astype(np.float32, copy=False)
            ...
            np.savez_compressed(
                ...
                source_detection_prob=source_detection_prob,
...
def main() -> None:
    ...
    parser.add_argument(
        "--appearance-embeddings-dir",
        type=str,
        default=None,
        help=(
            "V16.7 diagnostic only: save exact pre-edge neural appearance "
            "embeddings for each node in each adjacent-frame source/target role."
        ),
    )

    parser.add_argument(
        "--resume-appearance-cache",
        action="store_true",
        help="Skip a video when a valid appearance cache NPZ already exists.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Collect appearance embeddings and skip edge/division logits and "
            "all graph post-processing. Requires --appearance-embeddings-dir."
        ),
    )

    args = parser.parse_args()

    if args.embedding_cache_only and args.appearance_embeddings_dir is None:
        parser.error(
            "--embedding-cache-only requires --appearance-embeddings-dir"
        )
    ...
    appearance_embeddings_dir = (
        Path(args.appearance_embeddings_dir)
        if args.appearance_embeddings_dir is not None
        else None
    )
    ...
    config = PredictConfig(
        ...
        appearance_embeddings_dir=appearance_embeddings_dir,
        resume_appearance_cache=args.resume_appearance_cache,
    ...
    for fold in folds:
        print(
            "BioTrack3D++ V16.7 neural appearance cache: "
